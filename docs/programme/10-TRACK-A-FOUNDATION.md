...
| **A17** | Sandbox hardening | sandbox | A16 | Every threat in A16 mitigated. Currently **393 lines** carrying the platform's highest-consequence claim (**D009**) | Each A16 threat has a mitigation and a test. `process`, `require`, `fs`, `net` are provably unreachable from inside the isolate | WIP |
