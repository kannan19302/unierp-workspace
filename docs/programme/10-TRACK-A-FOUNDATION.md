...
| **A20** | Rate limiting, quotas and noisy-neighbour isolation | api, infra | — | Per-tenant, per-endpoint and per-principal limits; connection-pool and query-budget fairness | One tenant issuing a runaway report load leaves a second tenant's p95 within its SLO. Verified under load, not argued (**G-13**) | WIP |
