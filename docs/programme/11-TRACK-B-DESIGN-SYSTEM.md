...
| **B14** | Visual regression gate | B13 | Screenshot baselines per component × 7 themes × 2 densities × light/dark | An unintended visual change fails CI with a diff image. A deliberate one is approved by updating the baseline in the same commit | WIP |
