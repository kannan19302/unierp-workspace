...
| **B20** | Desktop surface | B18 | The desktop target (Flutter Windows, present in `unierp-mobile/windows`) treated as first-class: window chrome, menus, keyboard conventions, multi-window, native file dialogs | The desktop build runs the full navigation with platform-correct shortcuts and menu bar. Density defaults differ from mobile and are user-overridable | WIP |
