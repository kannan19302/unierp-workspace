...
| **B04** | Navigation — `Tabs`, `Breadcrumb`, `Pagination`, `SideNav`, `CommandPalette`, `Steps` | A02 | Navigation set consistent with the existing `layout` and `navigation` modules | Tabs support keyboard arrows + `Home`/`End`; the command palette is reachable via one shortcut from every page and searches routes, records and actions | WIP |
