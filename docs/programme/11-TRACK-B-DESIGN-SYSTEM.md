...
| **B02** | Overlays — `Tooltip`, `Popover`, `Menu`, `DropdownMenu`, `ContextMenu`, `Drawer`, `Sheet`, `Dialog` variants | A02 | A single focus-trap and portal layer underneath all of them | Focus is trapped and restored, `Esc` closes, nested overlays stack correctly, all reachable by keyboard alone, `axe` clean, no scroll-lock leak | WIP |
