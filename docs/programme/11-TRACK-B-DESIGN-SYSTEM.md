...
| **B17** | Adoption measurement | B15 | A report of design-system coverage per consuming repo: which screens compose from the package and which improvise | The report is generated in CI and published. Coverage may only rise. Any screen below the floor is named, so B's value is measured rather than assumed | WIP |
