...
| **B08** | Structure — `Accordion`, `Collapsible`, `SplitView`, `ResizablePanel`, `Card` variants, `DescriptionList`, `Timeline`, `TreeView`, `KanbanBoard` | A02 | The layout patterns that 890 pages currently improvise | `TreeView` handles 10k nodes with keyboard navigation and type-ahead; `SplitView` persists its position per user | WIP |
