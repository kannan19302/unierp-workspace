...
| **D18** | Settings import, export and environment promotion | D13 | Export a tenant's configuration and apply it to another tenant or environment, with a diff and a dry-run | Configuration is promoted sandbox → production with a reviewable diff and no secret leaving its scope | WIP |
