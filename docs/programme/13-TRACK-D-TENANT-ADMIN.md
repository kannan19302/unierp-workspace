...
| ID | Phase | Depends | Deliverable | Exit | Status |
| :- | :---- | :------ | :---------- | :--- | :----- |
| **D08** | Import framework | D01 | A generic staged import: upload → map → validate → dry-run → commit → reconcile, usable by any module rather than reimplemented per module | An import with 200 bad rows out of 10,000 reports every one actionably, imports nothing, and is re-runnable after correction | WIP |
