...
| **E03** | Tier assignment and depth targets | E02 | `docs/module-tier-manifest.json` reconciled with the audit: which modules must reach 3s, which are Tier-3, which carry logged Tier-4 exemptions | Every module has a target score and a stated reason. No module is silently exempt | WIP |
