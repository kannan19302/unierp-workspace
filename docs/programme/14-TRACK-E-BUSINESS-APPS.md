...
| **E33** | Semantic layer | A03 | A governed metric and dimension layer over the 1,836 models, so reports and dashboards do not hand-write SQL and break on the next migration (**G-7**) | A dashboard survives a schema migration that renames a column. Every metric has one definition | WIP |
