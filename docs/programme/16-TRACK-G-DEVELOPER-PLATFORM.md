...
| **G05** | Sandbox environments and data seeding | A18, C22 | Real sandbox tenants: create, refresh from production with masked data, reset, and promote to production | A developer refreshes a sandbox from production with all PII masked, provably, and promotes a change without touching production data | WIP |
