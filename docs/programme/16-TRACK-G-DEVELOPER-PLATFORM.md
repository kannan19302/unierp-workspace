...
| **G03** | Extension lifecycle | G01, A19 | Install, upgrade, rollback, disable, uninstall — with data migration, and per-tenant isolation of extension state | An upgrade that fails rolls back cleanly with no orphaned data. Uninstall leaves no residue and no dangling permissions | WIP |
