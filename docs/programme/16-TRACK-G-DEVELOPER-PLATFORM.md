...
| **G11** | Flow and workflow builder | G09, E05 | The `workflows/[id]` stub replaced: visual flow authoring with branching, loops, waits, human tasks, error paths and compensation | A flow with a failing external call compensates correctly and is resumable. Every flow run is inspectable step by step | WIP |
