...
| **G04** | Observability for customer code | A19 | Per-extension logs, traces, metrics, errors and resource consumption — visible to both the author and the tenant | An author debugs a failing handler from the portal without our involvement. A tenant sees what an installed extension is doing | WIP |
