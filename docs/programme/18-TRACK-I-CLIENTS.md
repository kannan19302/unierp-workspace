...
| **I11** | Mobile security | A24, D22 | Biometric unlock, secure storage, certificate pinning, jailbreak/root detection, screenshot policy, remote wipe, and enforced session policy | A tenant's MFA and session policy are enforced on mobile identically to web. Credentials are provably not in plain storage | WIP |
