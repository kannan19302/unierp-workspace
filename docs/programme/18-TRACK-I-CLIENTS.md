...
| **I04** | Token and theme adoption | B18, B19 | Generated Dart tokens consumed throughout; every hardcoded colour, spacing and type value removed | A hardcoded `Color(0x...)` or raw padding value fails `flutter analyze` via a custom lint. All 7 themes and both densities render | WIP |
...
| **I11** | Mobile security | A24, D22 | Biometric unlock, secure storage, certificate pinning, jailbreak/root detection, screenshot policy, remote wipe, and enforced session policy | A tenant's MFA and session policy are enforced on mobile identically to web. Credentials are provably not in plain storage | BLOCKED |
