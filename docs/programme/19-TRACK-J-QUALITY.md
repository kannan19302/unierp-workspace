...
| **J02** | Coverage that can fail — everywhere | A06 | Thresholds and `all: true` in every testable repo, with a ratchet that may only rise | Deleting any test file fails CI in every repo. Coverage floor recorded per repo and never lowered without a logged amendment (**D002**) | WIP |
