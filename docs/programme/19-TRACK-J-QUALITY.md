...
| **J06** | Integration test environment | A15 | Reproducible ephemeral environments with real Postgres, real migrations and seeded data — no mocked database in integration tests | Integration suites run against a real database in CI and locally with one command, and are not flaky across ten consecutive runs | WIP |
