...
| **K02** | Legal document set | — | Terms of service, acceptable use, SLA, DPA, sub-processor list, privacy policy, cookie policy — each reviewed and each published on the marketing site (H08) | Every document exists, is published, is versioned, and its acceptance is recorded per tenant with a timestamp | BLOCKED |
