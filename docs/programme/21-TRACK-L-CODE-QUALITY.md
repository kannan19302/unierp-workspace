...
| :- | :---- | :------ | :---------- | :--- | :----- |
| **L01** | Size and complexity gate | A02 | ESLint `max-lines`, `max-lines-per-function`, `complexity`, `max-depth`, `max-params` configured to the `CODE_STANDARDS § 4` table, with a recorded baseline per rule | Adding a 501-line service, an 81-line function, a complexity-16 branch, 5-deep nesting or a 6th parameter fails CI. Baseline recorded (86 files over the 1,000-line ceiling, largest 8,282) and may only fall (**D017**) | OPEN |
| **L02** | Naming convention gate | A02 | `@typescript-eslint/naming-convention` encoding `CODE_STANDARDS § 3` | A violation of the documented naming scheme fails CI on a new or modified file. Baseline recorded | OPEN |
