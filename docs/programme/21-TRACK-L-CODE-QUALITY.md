...
| **L04** | TODO discipline gate | A02 | A custom rule requiring every `TODO` to carry an issue reference and an owner, per `CODE_STANDARDS § 7` | A bare `TODO` fails CI. The 15 existing markers are each converted into a phase, an issue, or deleted | WIP |
