...
| **L13** | Remove the CI test exclusion | L12 | `*.coverage.spec.ts` no longer excluded when `process.env.CI` — the whole suite runs everywhere it runs | `vitest.config.ts` has an identical `exclude` list in CI and locally. The CI test count equals the local test count, asserted | WIP |
