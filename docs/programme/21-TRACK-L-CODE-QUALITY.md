...
| **L14** | Test-quality gate | L12, L13 | A gate rejecting the always-passing patterns as a class: assertion-free `it()` blocks, `toBeDefined()` as the sole assertion, catch-anything assertions, and skipped tests without an issue reference | Adding an assertion-free test fails CI. Adding `catch (e) { expect(e).toBeDefined() }` fails CI. **This is the gate whose absence allowed 194,494 lines of it** | WIP |
