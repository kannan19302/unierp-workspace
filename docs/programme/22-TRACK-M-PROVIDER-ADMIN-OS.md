...
| **M30** | Metering reconciliation, provider vs invoiced | M26, C14 | Reconciliation of provider-reported consumption against C14's metered quantities, with a variance report and drill-down to the events on both sides | A deliberate divergence between provider consumption and metered quantity is detected and reported with both sources named. Extends C14's reconciliation view; does not fork it | WIP |
