...
| **M24** | Capacity, scaling and performance | M21, C05 | Capacity models, headroom, autoscaling policy and load-driven scaling plans, extending C05's operations dashboard rather than replacing it | A capacity shortfall predicted by the model raises before it is hit, and the scaling plan it proposes executes on the pipeline. The prediction is checked against a real historical shortfall, not a synthetic one | WIP |
