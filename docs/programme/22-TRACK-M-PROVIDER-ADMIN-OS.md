...
| **M28** | Cost per tenant, margin and unit economics | M27, C14, C16 | Cost, revenue and margin per tenant, plan and module; cost per active user and per transaction; and the trend that shows whether a plan is priced above its cost | The gross margin of any tenant is stated with both sides traceable — cost to M25's ingested line, revenue to C16's invoice. A tenant whose cost exceeds its revenue is surfaced. **This is the phase K19 depends on** | WIP |
