...
| **M17** | Regions, residency and placement | M16, A26 | Regions as resources, with residency constraints, placement rules and the tenant→region binding that C22 migrations already assume | A resource whose placement violates a tenant's residency constraint is refused at plan time by an M08 policy, not at execution. Verified against a tenant with a hard residency constraint from A26 | WIP |
