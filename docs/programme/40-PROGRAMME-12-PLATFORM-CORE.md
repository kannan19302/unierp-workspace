...
| **P12-027** | Tenant column and RLS universality | P12-026 | Every tenant-scoped table carrying `tenantId` and an RLS policy, with exemptions individually justified | A table without both fails `check-rls-verify.mjs`. Every exemption has a recorded reason | WIP |
