...
| **P12-032** | Index policy and coverage | P12-026 | Indexes for every foreign key, tenant scope and common query shape, with an advisor for the rest | A foreign key without an index fails the gate. The advisor's proposals are measurably effective | WIP |
