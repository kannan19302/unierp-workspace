...
| **P12-008** | Standalone build for every claimed repository | P12-002 | Each repository building, typechecking, linting and testing from a clean clone with no sibling present | Every claimed repository builds standalone. A `workspace:*` escaping its workspace fails CI — closing **D008** | WIP |
