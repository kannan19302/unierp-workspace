...
| **P14-010** | Harness versioning | P14-009 | The harness versioned, so a plan state records which harness produced it | Every FINISH block records the harness version that wrote it; a version-less block fails the worklog check, proven by seeding one | OPEN |
...
| **P14-041** | Filesystem interaction tests | P14-020 | Tests for path handling across platforms, including Windows separators and spaces | A path containing a separator, a space or a non-ASCII character produces identical behaviour on every supported platform, verified by comparison | OPEN |
| **P14-042** | Cross-platform harness verification | P14-041 | The harness verified on the platforms agents actually run on | The harness suite passes on every supported platform in CI; a platform-specific failure blocks the merge rather than being skipped | OPEN |
...
| **P14-054** | Harness audit trail | P14-010 | Every harness action attributable to an agent, a version and a time | Any plan state change is attributable to an agent, a version and a time from the record alone; an unattributable change fails the audit check | OPEN |
...
| **P14-057** | Harness decomposition | P14-056 | The harness split into testable units where the census shows it is warranted | Each unit is independently testable; a unit reachable only by executing the whole script fails the decomposition check, proven by seeding one | OPEN |
...
| ID | Phase | Depends | Deliverable | Exit | Status |
| :- | :---- | :------ | :---------- | :--- | :----- |
| **P14-061** | The claim protocol, specified | P14-040 | The protocol written as a specification — states, transitions, invariants — rather than only as code | Every implemented behaviour maps to a specified one; an unspecified behaviour fails the conformance check, proven by adding one | OPEN |
...
| **P14-080** | Claim audit trail | P14-054 | Every claim, reset, release and finish attributable | Any phase's full claim history is reconstructible from the record alone, verified by comparison against the git history that produced it | OPEN |
...
| **P14-092** | Historical evidence parsing | P14-091 | The 194 existing transcripts parsed into the structured model, with unparseable ones reported | All 194 historical transcripts are parsed, or each unparseable one is listed by phase; a silently skipped transcript fails the check | OPEN |
| **P14-093** | Evidence completeness scoring | P14-092 | Each transcript scored: does it contain a command, its passing output, and its output when broken | The completeness distribution is published as counts; every transcript missing its deliberate-break section is named by phase, never aggregated away | OPEN |
| **P14-094** | Exit-criterion extraction | P14-091 | Extracting the runnable command from an exit criterion where one exists, and marking the rest as manual | All 4,571 phases are classified as machine-verifiable or manual and the ratio published; an unclassified phase fails the check | OPEN |
...
| **P14-096** | Sampling strategy | P14-095 | Risk-weighted sampling — money, isolation, auth and overridden phases sampled harder | The strategy is stated and its coverage computed as a percentage per risk class; a high-risk class at zero coverage fails the check | OPEN |
...
| **P14-114** | Evidence quality trend | P14-093 | Whether transcripts are getting better or worse over time | The trend is published as a measured series; a decline beyond the stated threshold fails the check and triggers a guidance review | OPEN |
...
| **P14-118** | Orphaned-commit detection | P14-117 | Commits that reference no phase, and phases that reference no commit | Both directions are reported as counts and ratcheted downward; an increase in either fails the ratchet, proven by seeding one | OPEN |
| **P14-119** | Evidence in CI | P14-095 | Re-verification running continuously rather than on demand | The sampler runs on its declared schedule; a missed run fails the freshness check, proven by suppressing one | OPEN |
...
| **P14-145** | Gate coverage of the standards | P14-131 | Every rule in `CODE_STANDARDS.md` mapped to a gate or explicitly marked unenforced | Every rule in CODE_STANDARDS.md maps to a gate or appears on the published unenforced list; a rule on neither fails the coverage check | OPEN |
...
| **P14-158** | Gate health dashboard | P14-135 | Gate firing rates, override rates, false positives and blocked work in one view | Every figure derives from measurement and drills to its source records; a figure with no derivation fails the dashboard check, proven by seeding one | OPEN |
...
| **P14-176** | Phase duration distribution | P14-175 | How long phases actually take, by programme and by kind | The measured duration distribution is published and replaces the planning estimate; a projection still citing the unmeasured estimate fails the check | OPEN |
...
| **P14-179** | Runner observability | P14-166 | Live visibility of what is running, where and how far along | An in-flight batch's phase, progress and elapsed time are answerable by command at any moment; a batch invisible to that command fails the check | OPEN |
...
| **P14-193** | Batch templates | P14-163 | Named batch shapes: a review batch, a programme batch, an audit batch | Each template executes end to end in test and its behaviour is asserted; a template with no passing test cannot be offered | OPEN |
| **P14-194** | Estimate correction | P14-176 | Replacing planning estimates with measured durations as data accumulates | Once the declared sample size is reached the published estimate is derived from measurement; an estimate still hand-set after that threshold fails the check | OPEN |
...
| **P14-197** | Brief size measurement | P14-196 | Every brief measured against the budget | The brief-size distribution is published; every brief exceeding the budget is named by phase, and the count is ratcheted downward | OPEN |
...
| **P14-203** | Prior-art detection | P14-202 | Detecting that a phase's deliverable may already exist before an agent starts | A phase whose exit criterion already passes is flagged at claim time rather than after the work; proven by seeding an already-satisfied phase | OPEN |
...
| **P14-224** | Flow metrics | P14-153 | Time from claim to finish, time blocked, time in review | Claim-to-finish, blocked and review durations are published per programme as measured distributions; a programme with no measurement fails the check | OPEN |
...
| **P14-235** | Metrics dashboard | P14-221 | One view of the whole programme's health, drilling to source | Every figure drills to the underlying records; a figure that cannot be traced to its source fails the dashboard check, proven by seeding one | OPEN |
...
| **P14-264** | Harness capacity limits | P14-016 | The declared ceiling on agents, phases and plan size, with behaviour at each | Each declared ceiling is reached in test and the behaviour at it matches the declaration; a ceiling never exercised fails the check | OPEN |
