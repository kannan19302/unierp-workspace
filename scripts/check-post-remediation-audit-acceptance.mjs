...
const reviewPath = resolve(workspaceRoot, "governance/FND-PA-002_POST_REMEDIATION_OWNER_REVIEW.md");
const auditPath = resolve(workspaceRoot, "governance/UNIERP_SAAS_READINESS_AUDIT_2026-09-03.md");
...
if (failures.length > 0) {
  console.error("FND-PA-002 post-remediation audit review is not accepted:");
  for (const failure of failures) console.error(`  - ${failure}`);
  process.exit(1);
}

console.log("FND-PA-002 post-remediation audit is accepted: 5 accountable authority decisions and audit digest verified.");
