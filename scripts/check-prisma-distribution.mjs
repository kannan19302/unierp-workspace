...
export function auditPrismaDistribution() {
  ...
  if (!existsSync(dataPkgPath)) {
    return { valid: false, violations: [`unierp-data package.json missing at ${dataPkgPath}`] };
  }
  ...
  for (const repoName of CONSUMER_REPOS) {
    ...
    if (dbDep && dbDep.replace(/^[\^~]/, "") !== canonicalDbVersion) {
      violations.push(`${repoName} @kannan19302/database version (${dbDep}) diverges from unierp-data (${canonicalDbVersion}).`);
    }

    if (prismaDep && prismaDep.replace(/^[\^~]/, "") !== canonicalPrismaVersion.replace(/^[\^~]/, "")) {
      violations.push(`${repoName} @prisma/client version (${prismaDep}) diverges from unierp-data (${canonicalPrismaVersion}).`);
    }
  }
