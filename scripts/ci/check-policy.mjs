...
const SKIP = new Set([
  ...
  'generated', 'storybook-static', 'migrations',
]);
...
const files = (dir, exts) => (existsSync(join(ROOT, dir)) ? walk(join(ROOT, dir), exts) : []);
const read = (f) => {
  try {
  ...
  },
  {
    ...
    scan() {
      ...
      const targets = [
        ...
        ...files('.husky', ['', '.sh']).filter((f) => !f.includes(`.husky${'/'}_`)),
      ];
      for (const f of targets) {
        read(f).split('\n').forEach((line, i) => {
          if (/^\s*#/.test(line)) return; // comments are allowed to mention it
          if (/continue-on-error:\s*true|--no-verify|\|\|\s*true\b/.test(line)) {
  ...
  },
  {
    ...
    scan() {
      const allowed = new Set([
        'README.md', 'PRD.md', 'TRD.md', 'APP_FLOW.md', 'UI_UX_BRIEF.md',
        'BACKEND_SCHEMA.md', 'IMPLEMENTATION_PLAN.md', 'ARCHITECTURE_REVIEW.md',
        'CODE_STANDARDS.md', 'CHANGELOG.md',
      ]);
    ...
    scan() {
      ...
      for (const f of files('apps/api/src', ['.controller.ts'])) {
        ...
        lines.forEach((line, i) => {
          ...
          if (/@Permissions\s*\(|@Public\s*\(|@SkipAuth\s*\(|@AllowAnonymous\s*\(/.test(window)) return;
          hits.push(`${relative(ROOT, f)}:${i + 1}  ${line.trim()}`);
        });
      }
      return hits;
    ...
    scan() {
      ...
      for (const f of targets) {
        read(f).split('\n').forEach((line, i) => {
          ...
          if (/#[0-9a-fA-F]{3,8}\b/.test(line) && !/--[a-z-]+\s*:/.test(line)) {
            hits.push(`${relative(ROOT, f)}:${i + 1}  ${line.trim().slice(0, 100)}`);
          }
        });
      }
      return hits;
    ...
    scan() {
      ...
      for (const f of targets) {
        read(f).split('\n').forEach((line, i) => {
          ...
          if (/:\s*[^;]*\b([2-9]|[1-9]\d+)px\b/.test(line) && !/--[a-z-]+\s*:/.test(line)) {
            hits.push(`${relative(ROOT, f)}:${i + 1}  ${line.trim().slice(0, 100)}`);
          }
        });
      }
...
for (const rule of HARD) {
  ...
  if (hits.length === 0) {
    ...
    for (const h of hits.slice(0, report ? 200 : 10)) console.log(`      · ${h}`);
...
for (const rule of RATCHET) {
  ...
  if (fresh || update) {
    ...
    exitCode = 1;
    console.log(`   ❌ ${rule.label}: ${was} → ${hits.length} (+${hits.length - was})`);
    console.log(`      ${rule.why}`);
    for (const h of hits.slice(0, report ? 200 : 5)) console.log(`      · ${h}`);
  } else if (hits.length < was) {
    console.log(`   ✅ ${rule.label}: ${was} → ${hits.length} (${hits.length - was})`);
